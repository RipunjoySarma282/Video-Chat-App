  ...
  .then((stream) => {
    ...
    Newpeer.on("call", (call) => {
      ...
      call.on("stream", (userVideoStream) => {
        ...
        addIdToMyVideo(video,h1,call.options.metadata.name)
      });
    ...
    socket.on("show_msg", (msg) => {
      ...
      $("ul").append(
        `<li class="message"><span class="user">${msg.username}</span><br/><span class="msg_txt">${msg.text}</span></li>`
      );
      scrollToBottom();
    });
  });
...
const connectToNewUser = (userId, stream, username) => {
  ...
  call.on("stream", (userVideoStream) => {
    ...
    addVideoStream(videoDiv,video,userVideoStream);
    addIdToMyVideo(video,h1,username)
  });
  call.on("close", () => {
    video.remove();
    h1.remove();
